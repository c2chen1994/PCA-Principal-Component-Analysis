...
def pca(X = np.array([]), no_dims = 50):
    ...
    a, b = np.linalg.eig(np.dot(np.transpose(X),X)) #a:eigenvalue b:aigenvector
    c = np.argsort(-a)[0:no_dims] #sort eigenvalue get top no_dims index
    M = b[:,c]  #get top no_dims eigenvectors
    Y = np.dot(X, M)
    return Y, M
...
def reconstruction_error(orig = np.array([]), decompressed = np.array([])):
    ...
    """TODO: write your code here"""
    error = np.linalg.norm(orig-decompressed) ** 2 / orig.shape
    return error
